...
template <class T> class ArraySecuence: Secuence<T>{
private:
...
public:
    ...
    void Append(T item) {
        array->addElement(array->len, item);
    }
    void Prepend(T item) {
    ...
    void InsertAt(T item, int index) {
        ...
        else if (index > 1 && index < array -> len - 1) {
    ...
    ArraySecuence<T>* operator+(ArraySecuence<T>* arr1) {
        ArraySecuence<T>* sumarr = new ArraySecuence(array -> len + arr1 -> len);
...
template <class T> class LinkedListSecuence: Secuence<T> {
private:
    LinkedList<T>* array;
public:
    LinkedListSecuence() {
